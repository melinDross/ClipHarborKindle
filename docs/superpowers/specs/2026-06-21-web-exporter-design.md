...
- `renderBookMarkdown(title, author, items, sourceFilename)` — genera el string Markdown final por libro, con auto-detección de idioma de salida basada en el idioma de los metadatos de cada entrada (mismo comportamiento que `detect_lang_for_book` / `kind_map` en Python).
